...
router.put("/:id", protect, async (req, res) => {
  try {
    ...
    await StockLedger.findOneAndUpdate(
      { entryRefId: stockEntryId },
      {
        product: updatedProductId,
        warehouse: updatedWarehouseId,
        quantity: updatedQty,
        entryType: updatedEntryType,
        direction: StockEntryTypes.includes(updatedEntryType)
          ? "IN"
          : "OUT",
      }
    );

    // Refetch updated StockEntry with populated fields
    ...
    console.error("Error updating stock entry and ledger:", error);
    res.status(400).json({ message: error.message });
  }
});
